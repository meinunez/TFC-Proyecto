...
# VLAN30 ↔ Acceso a VLAN10_Admin LDAP/Kerberos (TCP y UDP: 88, 464, 389, 636)
iptables -A FORWARD -i "$Interface_DMZ" -o "$Interface_Admin" -s "$VLAN30_DMZ" -d "$IP_LDAP" -p tcp -m multiport --dports 88,389,636,464 -j ACCEPT
iptables -A FORWARD -i "$Interface_DMZ" -o "$Interface_Admin" -s "$VLAN30_DMZ" -d "$IP_LDAP" -p udp -m multiport --dports 88,464 -j ACCEPT
...
echo "[ 5.3 ] Permitiendo tráfico entre cliente VPN y servidor LDAP/Kerberos..."
